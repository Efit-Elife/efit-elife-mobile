...
import { setSelectedFood, addToRecentSearches, selectSearchQuery, selectUIState } from '../store/foodSlice';
...
export default function FoodScreen() {
  ...
  const { selectedCategory } = useSelector((state: RootState) => selectUIState(state));
  ...
  const categoryInfiniteQuery = useInfiniteFoodsByCategory(selectedCategory || '');
  ...
  const handleFoodPress = useCallback((food: SimplifiedFoodItem) => {
  ...
  }, [currentSearchQuery, selectedCategory, searchInfiniteQuery, categoryInfiniteQuery, popularInfiniteQuery]);

  const { data, isLoading, error, hasNextPage, fetchNextPage, isFetchingNextPage } = currentQuery;

  // Monitor performance for lazy loading
  const totalItems = useMemo(() => {
    return data?.pages?.reduce((acc: number, page: any) => acc + page.hints.length, 0) || 0;
  }, [data]);
  ...
  const { headerTitle, headerSubtitle } = useMemo(() => {
    if (currentSearchQuery) {
      ...
      return {
        headerTitle: `${selectedCategory.charAt(0).toUpperCase() + selectedCategory.slice(1)} Foods`,
