...
const MealSection: React.FC<MealSectionProps> = ({ mealTime, entries, onAddFood, onRemoveEntry }) => {
  const dispatch = useDispatch();
  const summary = useSelector((state: RootState) => selectMealSummary(state, mealTime));
  ...
  const renderFoodEntry = ({ item }: { item: FoodLogEntry }) => (
    <View style={styles.foodEntry}>
      ...
      <TouchableOpacity 
        style={styles.removeButton}        onPress={() => {
          Alert.alert(
            'Remove Food',
            `Are you sure you want to remove "${item.foodName}" from your log?`,
            [
              { text: 'Cancel', style: 'cancel' },
              { text: 'Remove', style: 'destructive', onPress: () => onRemoveEntry(item.id) }
            ]
          );
        }}
      >
  ...
  return (
    <View style={styles.mealSection}>
      <View style={[styles.mealHeader, { backgroundColor: config.color + '10' }]}>
        ...
        <View style={styles.mealSummary}>
          <Text style={styles.mealCalories}>{Math.round(summary.calories)} cal</Text>
      ...
      >
        <Text style={[styles.addFoodText, { color: config.color }]}>+ Add Food</Text>
      </TouchableOpacity>
    </View>
  );
};
...
export const DailyFoodLogScreen: React.FC = () => {
  ...
  const currentDate = useSelector((state: RootState) => state.foodLog.currentDate);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <VStack className="flex-1">        {/* Header */}
        ...
        {/* Date Navigation */}        <HStack className="justify-between items-center p-4 bg-gray-50">
          <TouchableOpacity 
            onPress={() => {
        ...
        </HStack>        {/* Daily Summary */}
        <View style={styles.dailySummary}>
          <Text style={styles.summaryTitle}>Daily Total</Text>
          <View style={styles.summaryGrid}>
            <View style={styles.summaryItem}>
              <Text style={styles.summaryValue}>{Math.round(dailyLog.totalCalories)}</Text>
              <Text style={styles.summaryLabel}>Calories</Text>
            </View>
            <View style={styles.summaryItem}>
              <Text style={styles.summaryValue}>{Math.round(dailyLog.totalProtein)}g</Text>
              <Text style={styles.summaryLabel}>Protein</Text>
            </View>
            <View style={styles.summaryItem}>
              <Text style={styles.summaryValue}>{Math.round(dailyLog.totalCarbs)}g</Text>
              <Text style={styles.summaryLabel}>Carbs</Text>
            </View>
            <View style={styles.summaryItem}>
              <Text style={styles.summaryValue}>{Math.round(dailyLog.totalFat)}g</Text>
